...
"""
def x_minute_stats(X=None, stocks=None, clean_data = 0):
...
def stock_stats(X=None, stocks=None, start_date_string=start_date, end_date_string=end_date, clean_data = 0):
    ...
    if (X == None):
        filename = "stock_stats_" + str(stocks) + "_" + "Full" + start_date_string + "_" + end_date_string + ".csv"
    else:
...
def basic_daily_stats(X=None, stocks=None, start_date_string=start_date, end_date_string=end_date, clean_data = 0):
    ...
    #print(stock_data_table)
    if (X == 600):
        filename = "basic_daily_stats_" + "Full" + start_date_string + "_" + end_date_string + ".csv"
    else:
        filename = "stock_stats_" + str(stocks) + "_" + str(X) + "_min_" + start_date_string + "_" + end_date_string + ".csv"
    stock_data_table.to_csv(Path(filename))
    #columns = ["Ticker", "Date", "Length(days)", "Total Trades", "Mean Returns(Trades)"]
...
def stock_analysis(X=None, stocks=None, start_date_string=start_date, end_date_string=end_date):
    ...
    basic_daily_stats()
    # quotes_cleaner = TAQCleanQuotes(k=3, tau=0.0005)
    #
    # quotes_cleaner.cleanAllQuotes(tickers=["SUNW", "ADP"])
    #
    # trades_cleaner = TAQCleanTrades(k=3, tau=0.0005)
    #
